...
PPM: **INSPR-401** fixes host takeover of batch review, makes Review prominent and simplifies confirmation while retaining clock aging, identity expiry and measured layout. This tree prepares legacy SemVer **`0.2.1`** (`legacy-semver-public`) for **[inspr-at/flow-shell](https://github.com/inspr-at/flow-shell)**. Published **0.2.0** and earlier assets remain immutable. The candidate becomes published only after approved tag admission retains all four assets. `private: true` remains the npm publish guard.
