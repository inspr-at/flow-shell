...
PPM: **INSPR-426** adds an optional host-owned version slot while retaining escaped fallback, header actions and measured layout. This tree prepares legacy SemVer **`0.2.2`** (`legacy-semver-public`) for **[inspr-at/flow-shell](https://github.com/inspr-at/flow-shell)**. Published **0.2.1** and earlier assets remain immutable. The candidate becomes published only after approved tag admission retains all four assets. `private: true` remains the npm publish guard.
...
| Input | Effect |
|-------|--------|
| `slot="header-version"` | Optional host-owned version component rendered in the shell header outside shell buttons. Without assigned content, the shell retains the escaped `shellState.header.version` label. The host owns the slotted node and its listeners across shell rerenders; named-slot content is excluded from default body/toolbar fill measurement. |
...
| `--shell-notice-top` | Measured viewport position of status notices: current header bottom plus `NOTICE_TOP_GAP_PX` (`12px`). Updated on header resize, render, scroll, and notice display. `resolveNoticeTop(headerBottom, gap)` is an exported pure geometry helper; fixed chrome stays outside query containers. |
...
Compact header/footer rules use container queries on the internal `.shell-root` and `.shell-footer-root` wrappers (not `:host`), with responsive padding applied on `.shell-scaffold` and `.shell-footer-scaffold` descendants so a 342px content column gets compact treatment even when the viewport is wider. Viewport-fixed footer and notice chrome stay outside `.shell-root` so `container-type` layout containment cannot re-anchor fixed positioning. At narrow widths the header wraps as needed so app identity, project title, version/instance, and account controls stay readable. Long project, instance, and version labels truncate with ellipsis; full values remain on `title` attributes, `aria-label`, and existing control labels.
...
| `flow:review-batch` | no | Cancelable host takeover; without synchronous `preventDefault()`, opens the shell review dialog |
